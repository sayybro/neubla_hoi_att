...
class VCOCOEvaluator_orig():
    ...
    def __init__(self, preds, gts, subject_category_id, correct_mat):
        ...
        self.gts = []
        for img_gts in gts:
            img_gts = {k: v.to('cpu').numpy() for k, v in img_gts.items() if k not in ['id','type','dataset','img_id']}
            self.gts.append({
...
class VCOCOEvaluator():
    ...
    def __init__(self, preds, gts, subject_category_id, correct_mat, max_pred):
        self.overlap_iou = 0.5
        self.max_hois = max_pred
