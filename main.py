...
#     parser.add_argument('--output2', type=str, default='output2.txt')

#     # logging
#     parser.add_argument("--use_wandb", action="store_true", default=False)
#     parser.add_argument('--project_name', default='neubla')
#     parser.add_argument('--group_name', default='qpic_vaw')
#     parser.add_argument('--run_name', default='mtl_vaw')
...
#         train_stats = train_one_epoch(
#             model, criterion, data_loader_train, optimizer, device, epoch,
#             args.clip_max_norm,args.use_wandb, args)
#         lr_scheduler.step()
...
#                     if args.dataset_name == 'v-coco' or 'hico':
#                         test_stats,dataset_name = evaluate_hoi_att(args.dataset_file, model, postprocessors, dlv, args.subject_category_id, device, args)
                    
#                         #v-coco eval
#                         if 'v-coco' in args.dataset_name:
#                             log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
#                             **{f'test_{k}': v for k, v in test_stats.items()},
#                             'epoch': epoch,
#                             'n_parameters': n_parameters}    
...
#                         #hico eval                    
#                         elif 'hico' in args.dataset_name:
#                             log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
#                             **{f'test_{k}': v for k, v in test_stats.items()},
#                             'epoch': epoch,
#                             'n_parameters': n_parameters}
